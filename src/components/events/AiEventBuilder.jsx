...
export default function AiEventBuilder({ onBuilt }) {
  ...
  const handlePhoto = async (e) => {
    ...
    try {
      ...
      const response = await base44.functions.invoke('analyzeEventPhoto', { file_url });
          ...
          >
            ...
            <div className="flex-1">
              <p className="font-semibold text-sm flex items-center gap-1.5">
                ...
                {analyzing ? 'AI is building your event…' : 'Snap a photo, AI builds the event'}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {analyzing ? 'Analyzing the photo and filling in the fields' : 'Take a picture — AI fills in name, cadence, notes and more'}
              </p>
            </div>
          </button>
        </CardContent>
